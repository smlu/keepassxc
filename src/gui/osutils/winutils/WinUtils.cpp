        ...
        if (!allowScreenCapture()) {
            if (msg->message != WM_ENTERIDLE || DWORD(msg->wParam) == MSGF_DIALOGBOX) {
                ...
                if (!SetWindowDisplayAffinity(hwnd, WDA_EXCLUDEFROMCAPTURE)) {
                    qCritical().nospace()
                        << "Couldn't set WDA_EXCLUDEFROMCAPTURE display affinity flag to window. error_code: 0x" << hex
                        << GetLastError();
                    if (getMainWindow()) {
                        const int showTime = []() {
#if defined(KEEPASSXC_BUILD_TYPE_SNAPSHOT) || defined(KEEPASSXC_BUILD_TYPE_PRE_RELEASE)
                            // If main window is not visible we assume it has not been
                            // fully initialized yet, and we delay showing the message because
                            // on pre-release version some message could already be displayed.
                            return getMainWindow()->isVisible() ? 0 : MessageWidget::DefaultAutoHideTimeout;
#else
                            return 0;
#endif
                        }();
                        QTimer::singleShot(showTime, []() {
                            getMainWindow()->displayGlobalMessage(
                                tr("WARNING: Application screen capture protection is disabled!"),
